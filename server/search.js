...
function search(channelData, items) { //test on seodam.json
    const TEST_ITEMS = ["condensed milk", "chocolate"];
    channelData.forEach(page => {
...
// using page token - https://www.googleapis.com/youtube/v3/playlistItems?part=snippet%2CcontentDetails&maxResults=50&playlistId=UU84Zkx_92divh3h4sKXeDew&key=AIzaSyDLKhdkdQn8cQrd5lOYMNtDKqTacxhOs6c&pageToken=CGQQAA
    ...
    until (
        ...
        function done(err) {
            if (err) {
                ...
                res.json(collapseAndNeaten(answers));
            }
        }
    );
